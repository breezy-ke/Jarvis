...
For email, the next section shows how this holds up against real attack
patterns, and the suite that proves it.

## Email

An email says whatever its sender wants, including instructions aimed at
Jarvis. Here's what an attacker might try, and what stops it:

| They try to | What stops it |
|---|---|
| **Make Jarvis send, forward or reply** | `email.send` always waits for your approval, then 60 seconds for undo, and sends exactly the bytes you approved (the approval is bound to a hash). The drafting model has no tools and writes only the body. |
| **Add themselves to a reply** (cc, bcc, Reply-To) | Recipients, subject and threading headers are computed by code from the conversation, whatever the email or the model says. A Reply-To is honoured, but anyone who isn't on the conversation and isn't someone you know is flagged "New recipient, check carefully", and nothing goes to them, not even a Gmail draft, without you. |
| **Give the AI orders** (hidden text, fake system prompts, encoded or translated instructions) | The sorting and drafting models have no tools, and see the email only inside nonce-tagged untrusted markers. Code looks at what you can't see: text hidden by CSS, HTML comments, invisible Unicode "tag" characters, and a plain-text part that says something the page doesn't. Any of those makes the email **suspicious**: no automatic draft, no alert. Visible "ignore your instructions" phrasing, even split by invisible characters or hidden in base64, gets a warning chip. |
| **Pretend to be someone** | A contact's name from another address (look-alike letters included), your own name from someone else's address, or failing the sender checks Gmail records (DMARC, or SPF without a valid DKIM signature) all make the email suspicious. |
| **Phish with links** | IP-address, punycode and shortened links are flagged; `javascript:` and `data:` links, even behind a button, make the email suspicious. The app shows email as plain text: no remote images, nothing clickable from the email's HTML. |
| **Steer the chat agent** | Once an email (or a forwarded Telegram message) is in a conversation, Jarvis won't change its memory there, and everything it proposes waits for you, even actions that normally run on their own (notifications, labels, calendar holds). |
| **Plant a "fact"** | Email is never mined into memory. Anything Jarvis picks up from a conversation waits for you to confirm it on the What I know page. |
| **Make your phone buzz** | By default, instant alerts come only for email from people you know (`config/email.yaml`), never in quiet hours, at most six an hour, and only ever to you. |
| **Mess up your Gmail** | Labels are limited to `Jarvis/…` by the action's schema. Jarvis has no archive, delete or spam action. |

**Proof:** the injection suite (`evals/email_injection/attacks.yaml`) runs 61
attack emails through the whole pipeline in CI: sync, sorting, labels,
automatic drafts and their Gmail copies, alerts, the send queue, and the chat
agent asked to handle the email. It runs each one twice: with the normal test
model, and with an obedient model that does whatever the email says, with
every tool it has. Every attack must end with nothing sent, no recipient from
outside the conversation used, only `Jarvis/…` labels touched, memory
unchanged and no calendar event, and 100% must pass. Switching off any one
defence makes it fail.

**Sending at most once.** If Gmail's answer to a send is lost, the send is
marked "unknown outcome", never retried. Jarvis marks every email it sends
with an `X-Jarvis-Action` header, and confirms the send only when that email
shows up in your Sent mail with the recipients you approved.

**Stored email.** Subjects, names, snippets, bodies, attachment names,
summaries, tasks, dates and Jarvis's drafts are encrypted with
`JARVIS_SECRET_KEY`. Addresses, dates and labels stay
readable, for searching and sorting. Email text older than 90 days is removed
...
- Google scopes: `gmail.modify` (read, label, draft and send; it doesn't
  allow permanent deletion), `calendar.readonly`, and `calendar.events` for
  the private holds you tap. Sending always waits for your approval.
...
- **An email can still fool you.** Jarvis flags what code can check, but a
  ...
  Jarvis's summary of it. Check the sender before acting on anything about
  money or passwords.
